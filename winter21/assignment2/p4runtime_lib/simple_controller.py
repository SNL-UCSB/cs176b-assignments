...
def program_switch(addr, device_id, sw_conf_file, workdir, proto_dump_fpath):
    ...
    try:
        ...
        # readTableEntries(sw, 'MyIngress.ipv4_lpm', p4info_helper)
...
def readTableEntries(sw, table_name, p4info_helper):
    table_id = p4info_helper.get_tables_id(table_name)
    print "Reading Table Entries for {}".format(table_name)
    for x in sw.ReadTableEntries(table_id=table_id):
        print x
    return
